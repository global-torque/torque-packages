...
function assertPlainObjectProperties(value: object): void {
  ...
  for (const [key, descriptor] of Object.entries(Object.getOwnPropertyDescriptors(value))) {
    if ('get' in descriptor || 'set' in descriptor) {
      throw new TypeError(`Canonical JSON does not accept accessor properties: ${key}.`);
    }
  }
}
...
function assertDenseArray(value: unknown[]): void {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (!descriptor) throw new TypeError('Canonical JSON does not accept sparse arrays.');
    if ('get' in descriptor || 'set' in descriptor) {
  ...
  for (const key of Object.getOwnPropertyNames(value)) {
    ...
    throw new TypeError(`Canonical JSON does not accept array property: ${key}.`);
  }
}
...
function encodeProgrammatic(value: unknown, active: Set<object>): string {
  ...
  if (active.has(value)) throw new TypeError('Canonical JSON does not accept cycles.');
  active.add(value);
  try {
...
class RawJsonParser {
  ...
  public parse(): unknown {
    ...
    this.skipWhitespace();
    if (this.index !== this.input.length) throw new TypeError('Canonical JSON raw input has trailing data.');
    return value;
  }
  ...
  private parseValue(): unknown {
    ...
    throw new TypeError(`Canonical JSON raw input has an invalid token at character ${this.index}.`);
  }
  ...
  private parseString(): string {
    ...
    while (this.index < this.input.length) {
      ...
      if (codeUnit < 0x20) throw new TypeError('Canonical JSON raw strings contain an unescaped control character.');
      if (escaped) {
        ...
        if (this.input[this.index] === 'u') {
          if (!/^[0-9a-f]{4}$/iu.test(this.input.slice(this.index + 1, this.index + 5))) {
            throw new TypeError('Canonical JSON raw strings contain an invalid Unicode escape.');
          }
          this.index += 5;
          escaped = false;
          continue;
        }
        throw new TypeError('Canonical JSON raw strings contain an invalid escape.');
      }
      if (codeUnit === 0x5c) {
        escaped = true;
        this.index += 1;
    ...
    throw new TypeError('Canonical JSON raw string is unterminated.');
  }
  ...
  private parseInteger(): number {
    ...
    if (this.input[this.index] === '0') {
      ...
      if (this.input[this.index] >= '0' && this.input[this.index] <= '9') {
        throw new TypeError('Canonical JSON raw integers may not contain leading zeroes.');
      }
    } else {
      if (!/[1-9]/u.test(this.input[this.index] ?? '')) throw new TypeError('Canonical JSON raw input has an invalid integer.');
      while (this.input[this.index] !== undefined && /\d/u.test(this.input[this.index]!)) this.index += 1;
    }
    const token = this.input.slice(start, this.index);
    if (token === '-0') throw new TypeError('Canonical JSON raw input does not accept negative zero.');
    if (this.input[this.index] === '.' || this.input[this.index] === 'e' || this.input[this.index] === 'E') {
      throw new TypeError('Canonical JSON raw input accepts safe integers only.');
    }
    const integer = BigInt(token);
    if (integer < BigInt(-MAX_SAFE_INTEGER) || integer > BigInt(MAX_SAFE_INTEGER)) {
      throw new TypeError('Canonical JSON raw integer is outside the safe integer range.');
    }
  ...
  private parseArray(): unknown[] {
    ...
    while (true) {
      ...
      if (this.input[this.index] !== ',') throw new TypeError('Canonical JSON raw array is malformed.');
      this.index += 1;
    }
  }
  ...
  private parseObject(): Record<string, unknown> {
    ...
    while (true) {
      this.skipWhitespace();
      if (this.input[this.index] !== '"') throw new TypeError('Canonical JSON raw object key is missing.');
      const key = this.parseString();
      if (keys.has(key)) throw new TypeError(`Canonical JSON raw object contains duplicate key: ${key}.`);
      keys.add(key);
      this.skipWhitespace();
      if (this.input[this.index] !== ':') throw new TypeError('Canonical JSON raw object is missing a colon.');
      this.index += 1;
      ...
      if (this.input[this.index] !== ',') throw new TypeError('Canonical JSON raw object is malformed.');
      this.index += 1;
    }
  }
}
...
export const canonicalJsonFromRaw = (raw: string): string => {
  if (typeof raw !== 'string') throw new TypeError('Canonical JSON raw input must be a string.');
