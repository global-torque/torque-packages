...
export interface BuildWorkboxRuntimeCachingOptions {
  excludeNavigationPathPrefixes?: string[];
}
...
const normalizeNavigationPathPrefix = (value: string) => {
...
const normalizeNavigationPathPrefixes = (prefixes: readonly string[] = []) => (
  [...new Set(prefixes.map(normalizeNavigationPathPrefix).filter(Boolean))]
);
...
const getServiceRelativePathname = (
...
const matchesAnyPathnamePattern = (
  pathnames: string[],
  pattern: PathPattern,
...
const createNavigationMatcher = (excludedPathPrefixes: readonly string[]) => {
...
const createNavigationFallbackPlugin = (excludedPathPrefixes: readonly string[]) => {
  const shouldIncludeDashboardShellFallback = !excludedPathPrefixes.some((prefix) => (
    matchesPathPrefix('/dashboard', prefix)
  ));
...
const createNavigationRule = (
  policy: ResolvedOfflineDomainPolicy,
  options: BuildWorkboxRuntimeCachingOptions,
): RuntimeCacheRule => {
...
export const buildWorkboxRuntimeCaching = (
  ...
  resolveOfflineDomainPolicies(env).filter(policy => policy.scope === 'public').map((policy) => (
