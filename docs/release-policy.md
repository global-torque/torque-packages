...
The candidate's ordinary immutable release tag is `framework-v0.2.4` for the
`0.2.4` candidate. Dispatch the candidate workflow from that tag so its
attestations carry `refs/tags/framework-v0.2.4`. The prior `framework-v0.2.3`
