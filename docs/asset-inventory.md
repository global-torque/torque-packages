...
This is the source and rights inventory for framework candidate `0.2.4`.
Counts cover regular asset files (`svg`, `webp`, raster images, and font
files) under the seven framework packages.

The 0.2.4 SFC correction changes no framework asset paths; the retained
0.2.3 inventory remains the asset baseline for this candidate.
