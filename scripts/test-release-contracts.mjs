...
const publisherCandidate = '0.4.10';
...
assert.deepEqual(
  ...
  ['./release/global-torque-domain-types-0.4.10.tgz'],
);
assert.equal(
...
const dependencyMap = {
  '@global-torque/domain-types': {},
  '@global-torque/invest-core': { '@global-torque/domain-types': '0.4.10' },
  '@global-torque/invest-data': { '@global-torque/domain-types': '0.4.10', '@global-torque/invest-core': '0.4.10' },
  '@global-torque/invest-runtime': { '@global-torque/domain-types': '0.4.10', '@global-torque/invest-core': '0.4.10', '@global-torque/invest-data': '0.4.10' },
  '@global-torque/invest-widgets': { '@global-torque/domain-types': '0.4.10', '@global-torque/invest-core': '0.4.10' },
  '@global-torque/invest-features': { '@global-torque/domain-types': '0.4.10', '@global-torque/invest-core': '0.4.10', '@global-torque/invest-data': '0.4.10', '@global-torque/invest-runtime': '0.4.10' },
  '@global-torque/invest-shell': { '@global-torque/domain-types': '0.4.10', '@global-torque/invest-core': '0.4.10', '@global-torque/invest-runtime': '0.4.10', '@global-torque/invest-widgets': '0.4.10', '@global-torque/invest-features': '0.4.10' },
};
...
function makeCandidate(directory) {
  ...
  for (const [index, name] of packageNames.entries()) {
    ...
    const archiveName = `${name.slice(1).replace('/', '-')}-0.4.10.tgz`;
    ...
    const entry = {
    name, version: '0.4.10', directory: `packages/${name.slice('@global-torque/'.length)}`,
    ...
    writeJson(`${archive}.manifest.json`, {
      schemaVersion: 1, package: name, version: '0.4.10', sourceRepository: 'fixture/torque-packages',
      sourceRevision: null, sourceDirty: true, sourcePackageRepository: 'fixture/source', sourcePackageRevision,
  ...
  const receipt = {
    schemaVersion: 1, candidate: '0.4.10', sourceRepository: 'fixture/torque-packages', sourceRevision: null,
...
function expectPackCandidateFailure(label, { candidate = '0.4.10', mode, existingOutput = false }) {
  ...
  assert.equal(
    ...
    !existingOutput && candidate === '0.4.10',
    `${label} must stop before build when eligibility or output checks fail`,
  );
  if (mode === 'check-failure' && !existingOutput && candidate === '0.4.10') {
...
function makeWorkflowFixture(label) {
  ...
  writeJson(path.join(directory, 'tag-ref.json'), {
    ref: 'refs/tags/framework-v0.4.10',
...
expectWorkflowFailure('workflow-other-package-byte-mismatch', {
  mutateAsset: fixture => fs.appendFileSync(path.join(fixture.releaseAssets, 'global-torque-domain-types-0.4.10.tgz'), 'tamper'),
...
const writeContextFixture = ({ run = {}, release = {}, tagRef = {} } = {}) => {
  writeJson(path.join(contextDirectory, 'run.json'), { status: 'completed', conclusion: 'success', event: 'workflow_dispatch', headBranch: 'framework-v0.4.10', headSha: 'd'.repeat(40), workflowName: 'Framework package candidate', ...run });
  writeJson(path.join(contextDirectory, 'release.json'), { id: 99, tag_name: 'framework-v0.4.10', draft: false, prerelease: false, immutable: true, assets: expectedContextAssets.map((name, index) => ({ id: index + 1, name })), ...release });
  writeJson(path.join(contextDirectory, 'tag-ref.json'), { ref: 'refs/tags/framework-v0.4.10', object: { type: 'commit', sha: 'd'.repeat(40) }, ...tagRef });
...
writeContextFixture();
runScript('verify-provenance-context.mjs', [path.join(contextDirectory, 'candidate-receipt.json'), '67890', 'framework-v0.4.10', 'fixture/torque-packages'], { env: contextEnv });
for (const [label, fixture] of [
  ...
  expectFailure(label, () => runScript('verify-provenance-context.mjs', [path.join(contextDirectory, 'candidate-receipt.json'), '67890', 'framework-v0.4.10', 'fixture/torque-packages'], { env: contextEnv }));
}
...
writeContextFixture();
expectFailure('receipt-identity', () => runScript('verify-provenance-context.mjs', [path.join(contextDirectory, 'candidate-receipt.json'), '67890', 'framework-v0.4.10', 'fixture/torque-packages'], { env: contextEnv }));
