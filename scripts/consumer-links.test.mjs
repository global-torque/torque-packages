...
function fixture(t) {
  ...
  for (const name of FRAMEWORK_PACKAGE_NAMES) {
    ...
    fs.writeFileSync(path.join(directory, 'package.json'), JSON.stringify({ name, version: '0.4.11' }));
  }
...
test('recovery accepts an intentional current cohort update', t => {
  ...
  fs.writeFileSync(appManifest, JSON.stringify({
    name: 'fixture-app',
    dependencies: Object.fromEntries(FRAMEWORK_PACKAGE_NAMES.map(name => [name, '0.4.11'])),
  }));
  installRegistryFramework(consumer, { version: '0.4.11' });
  assert.equal(recover(consumer, { install: false }).recovered, true);
});
